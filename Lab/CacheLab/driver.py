...
def computeMissScore(miss, lower, upper, full_score):
    ...
    range_ = (upper - lower) * 1.0  # 避免与内置函数 range 冲突
...
def main():
    ...
    stdout_data = p.communicate()[0].decode('utf-8')  # bytes → str
    ...
    csim_cscore  = list(map(int, resultsim[0:1]))  # map() 在 Python3 中是惰性的，需转为 list
